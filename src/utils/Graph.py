...
class Graph:
    ...
    def add_vertex(self, new_vertex):
        if self.adjacency_list.get(new_vertex) is None:
            self.adjacency_list.set(new_vertex, [])

    def add_directed_edge(self, from_vertex, to_vertex, weight=1.0):
        self.edge_weights.set((from_vertex, to_vertex), weight)
        self.adjacency_list.get(from_vertex).append(to_vertex)
    ...
    def get_edge_weight(self, from_vertex, to_vertex):
        distance = self.edge_weights.get((from_vertex, to_vertex))
        return distance
